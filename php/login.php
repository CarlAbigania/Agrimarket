    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $hashed_password)) {
            ...
            if ($user['role'] == 'customer') {
                header("Location: ../Agrimarket/home.php");  // Redirect to customer page
            } elseif ($user['role'] == 'admin') {
                header("Location: ../admin_side/admin_panel.php");  // Redirect to admin panel
            }
            exit();
        } else {
            // Invalid login: incorrect password
            echo "<script>alert('Invalid email or password.'); window.location.href='../userlogin/login.html';</script>";
        }
    } else {
        // Invalid login: no user found
        echo "<script>alert('Invalid email or password. No user found'); window.location.href='../userlogin/login.html';</script>";
    }
